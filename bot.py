...
user_pairs = {}
user_notifications = {}  # chat_id -> True/False (включена ли рассылка)
...
async def send_tg(text, chat_id, reply_markup=None):
...
def get_main_menu_keyboard(mode, threshold, notifications_on=True):
    status_text = "ON" if notifications_on else "OFF"
    status_emoji = "🔔" if notifications_on else "🔕"
    
    buttons = [
        [
            {"text": f"🔹 Strong ({len(PAIRS_STRONG)})", "callback_data": "mode_Strong"},
            {"text": f"🔹 Meme ({len(PAIRS_MEME)})", "callback_data": "mode_Meme"},
            {"text": f"🔹 All ({len(PAIRS_ALL)})", "callback_data": "mode_All"}
        ],
        [
        ...
        [{"text": "🔹 Show my pairs", "callback_data": "show_pairs"}]
    ]
    ...
    for row in buttons:
        for btn in row:
            if btn["text"].startswith("✅"):
                continue
            if btn["text"].split()[0] if len(btn["text"].split()) > 0 else "" in ["Strong", "Meme", "All"]:
                btn_name = btn["text"].split()[1] if len(btn["text"].split()) > 1 else ""
                if mode == btn_name:
                    btn["text"] = f"✅ {btn['text']}"
...
def get_pair_selection_keyboard(chat_id):
    current_pairs = user_pairs.get(chat_id, [])
    buttons = []
    
    all_pairs = get_pairs_for_mode(CURRENT_MODE)
    for pair in all_pairs:
        is_selected = pair in current_pairs
        text = f"<font color='green'>🔹 {pair}</font>" if is_selected else f"🔹 {pair}"
        buttons.append([{"text": text, "callback_data": f"toggle_{pair}"}])
    
    buttons.append([{"text": "🔹 Back to main", "callback_data": "back_main"}])
    return {"inline_keyboard": buttons}
...
async def handle_callback(callback_query):
    ...
    if data == "back_main":
        ...
        await send_tg("🔹 Main menu:", chat_id, kb)

    elif data == "toggle_notifications":
        current_status = is_notifications_enabled(chat_id)
        user_notifications[chat_id] = not current_status
        save_data()
        
        status_text = "ON" if user_notifications[chat_id] else "OFF"
        status_emoji = "🔔" if user_notifications[chat_id] else "🔕"
        await send_tg(f"🔹 Notifications: {status_emoji} {status_text}", chat_id)
        
        current_thresh = user_thresholds.get(chat_id, DEFAULT_MIN_PROBABILITY)
        kb = get_main_menu_keyboard(CURRENT_MODE, current_thresh, user_notifications[chat_id])
        await send_tg("🔹 Main menu:", chat_id, kb)

    elif data == "show_pairs":
        pairs = get_pairs_for_user(chat_id)
        if not pairs:
            pairs = get_pairs_for_mode(CURRENT_MODE)
        text = f"🔹 Your pairs ({len(pairs)}):\n\n{format_pair_list(pairs)}\n\nClick a pair to toggle selection."
        kb = get_pair_selection_keyboard(chat_id)
        await send_tg(text, chat_id, kb)
        ...
        pairs = user_pairs[chat_id] if user_pairs[chat_id] else get_pairs_for_mode(CURRENT_MODE)
        text = f"🔹 Your pairs ({len(pairs)}):\n\n{format_pair_list(pairs)}\n\nClick a pair to toggle selection."
        kb = get_pair_selection_keyboard(chat_id)
        await send_tg(text, chat_id, kb)
        ...
        if chat_id not in user_pairs or not user_pairs[chat_id]:
            pass
        ...
        notif_status = is_notifications_enabled(chat_id)
        kb = get_main_menu_keyboard(CURRENT_MODE, current_thresh, notif_status)
        await send_tg(f"🔹 Mode: {mode}\n🔹 Pairs: {len(get_pairs_for_mode(mode))}", chat_id, kb)

    elif data == "set_threshold":
        waiting_for_threshold[chat_id] = True
        await send_tg("🔹 Enter number (10-99) for minimum probability threshold:", chat_id)

    elif data == "info":
        info = ("⚠️ <b>IMPORTANT:</b>\n\n"
                ...
                "🔹 Filter: >= user threshold\n\n"
                "🔹 You can select specific pairs to monitor.\n"
                "🔹 You can enable/disable notifications anytime.")
        await send_tg(info, chat_id)

    elif data == "stop":
        active_users.discard(chat_id)
        save_data()
        await send_tg("🔹 You have been unsubscribed from signals.\n"
                      "Send /start to resume.", chat_id)
...
def detect_flag(high, low, close, volume, atr):
    ...
    if not (impulse_up or impulse_dn): return None
...
async def scan_signals():
    ...
    while BOT_RUNNING:
        ...
        for uid in active_users:
            if not is_notifications_enabled(uid):  # Проверяем, включена ли рассылка
                continue
                ...
                for sym, res in zip(batch, results):
                    ...
                    for signal in patterns:
                        ...
                        for user_id in list(active_users):
                            if not is_notifications_enabled(user_id):  # Пропускаем, если рассылка выключена
                                continue
                            ...
                            if chat_id in waiting_for_threshold:
                                try:
                                    ...
                                    if 10 <= val <= 99:
                                        ...
                                        save_data()
                                        await send_tg(f"Threshold set: {val}%", chat_id)
                                    else:
                                        await send_tg("Number must be 10-99.", chat_id)
                                except:
                                    await send_tg("Enter a valid number.", chat_id)
                                waiting_for_threshold.pop(chat_id, None)
                            else:
                                if text.lower() == "/start":
                                    active_users.add(chat_id)
                                    if chat_id not in user_notifications:
                                        user_notifications[chat_id] = True  # Включаем рассылку по умолчанию
                                    save_data()
                                    current_thresh = user_thresholds.get(chat_id, DEFAULT_MIN_PROBABILITY)
                                    notif_status = is_notifications_enabled(chat_id)
                                    kb = get_main_menu_keyboard(CURRENT_MODE, current_thresh, notif_status)
...
async def main():
    ...
    try:
        await asyncio.gather(web_task, scan_task, listen_task)
